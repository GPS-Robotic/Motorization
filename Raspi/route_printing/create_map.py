...
#				 with ',' being the data-seperator (can be adjusted) (NOTE: '\n' = new line)
#		OPTIONAL:
#		output_file_name:	file-name of output-HTML (i.e. "log.html"),
#				  	if not set, will be "" --> output file-name will be input filename + '.html'
#		input_path:		path of input file (i.e. "./track"),
#					if not set, will be "./GPS_route_log"
#		output_path:		path of output file (i.e. "./route_htmls"),
#					if not set, will be "../route_htmls",
#					output-path will be created, if not existent!
#		data_seperator:		tells, by which symbol the data in the input file is seperated,
#					if not set, will be ','
...
def make_route_HTML(input_file_name, output_file_name="", input_path="./GPS_route_log", output_path="../route_htmls", data_seperator=",", open_at_finish=False):

	track_file = open(input_path + "/" + input_file_name, 'r')

	GPS_data = []
	for line in track_file:
		data = (line.split("\n")[0]).split(data_seperator)
